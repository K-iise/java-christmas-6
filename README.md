...
중복된 할인과 증정을 통해서 고객들에게 해택을 제공하는 것.
...
메뉴 여부 확인
- 음료만 주문 시, 주문할 수 없다, 또한 메뉴는 한 번에 최대 20개 까지 주문 할 수 있다(총 개수는 상관 x).

1. 12월 이벤트 할인 여부 판단

크리스마스 디데이 할인
- 12월 1일 부터 12월 25일 까지 진행하며 1000원으로 시작해서 날마다 할인 금액이 100원씩 증가한다. 총 주문 금액에서 해당 금액만큼 할인

평일 할인
- 평일(일요일~목요일)에는 디저트 메뉴를 1개당 2023원 할인한다.

주말 할인
- 주말(금요일,토요일)에는 메인 메뉴를 메뉴 1개당 2023원 할인한다.

특별 할인
이벤트 달력에 별이 있는 (3, 10, 17, 24, 25, 31)일의 경우는 총 주문 금액에서 1000원 할인한다.

이벤트 기간은 크리스마스 디데이 할인을 제외한 다른 이벤트는 1일 부터 31까지 적용한다.

2. 증정 이벤트 여부 판단
할인 전 총 주문 금액이 12만 원 이상일 때, 샴폐인 1개를 증정한다.

3. 12월 이벤트 배지 부여
혜댁 받은 금액에 따라 아래의 이벤트 배지를 부여한다. 배지는 2024 새해 이벤트에서 활용되고, 새해 이벤트 참여시 배지에 따라 새해 선물을 증정한다.
